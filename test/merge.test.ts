...
function item(partial: Partial<RankedItem> & { id: string; url: string }): RankedItem {
  return { source: 'tavily', title: 't', snippet: '', ageHours: null, relevance: 0.5, ranked: true, freshness: 0.5, position: 1, engines: ['tavily'], ...partial };
}
...
describe('mergeItems', () => {
  ...
  it('folds the same URL from a second engine into one row', () => {
    const a = item({ id: 'a', url: 'https://x.com/p/1', relevance: 0.6, position: 3, engines: ['tavily'] });
    const b = item({ id: 'b', url: 'https://www.x.com/p/1/', relevance: 0.9, position: 1, engines: ['naver'], ageHours: 5, freshness: 0.9 });
    const c = item({ id: 'c', url: 'https://x.com/p/2', engines: ['naver'] });
    const out = mergeItems([a], [b, c]);
    expect(out).toHaveLength(2);
    expect(out[0]).toMatchObject({ id: 'a', relevance: 0.9, position: 1, engines: ['tavily', 'naver'], ageHours: 5, freshness: 0.9 });
