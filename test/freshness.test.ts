...
describe('structured publication dates', () => {
  ...
  it('displays calendar dates without implying hour precision', () => {
    ...
    expect(formatPublicationAge(resolvePublication('2026-09-18T17:30:00Z', '', now))).toBe('1시간 전');
    expect(formatPublicationAge({ ageHours: null })).toBeNull();
    expect(formatPublicationAge({ ageHours: 72 })).toBe('3일 전');
  });
});

describe('parseAgeHours', () => {
  it('reads relative prefixes from 검색 공급자 snippets', () => {
...
import { plainText as decodeEntities } from '@/lib/search-providers';
