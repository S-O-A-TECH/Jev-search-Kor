...
describe('canonicalUrl', () => {
  it('한국어 자료의 언어·검색어·경로 대소문자를 보존한다', () => {
    expect(canonicalUrl('https://example.org/Article?lang=ko&s=역사&t=2026'))
      .not.toBe(canonicalUrl('https://example.org/article?lang=en&s=과학&t=2025'));
  });
...
describe('clusterItems', () => {
  ...
  it('orders by the shown percentage, then engine agreement, then engine rank', () => {
    ...
    const b = item({ id: 'b', relevance: 0.78, freshness: 1, position: 1, engines: ['tavily', 'naver'] });
