...
export function buildCandidates(request: string): string[] {
  if (/[가-힣ㄱ-ㅎㅏ-ㅣ]/.test(request)) {
    // Propose only a conservative trailing politeness removal; preserve the original.
    // Never split Korean proper nouns, particles, dates, negation or short quiz answers.
    const original = request.trim();
    const shorter = original.replace(/\s+(?:알려\s?줘|알려주세요|찾아\s?줘|검색해\s?줘|설명해\s?줘)[.!?？]*$/u, '').trim();
    return shorter && shorter !== original ? [original, shorter] : [original];
  }
