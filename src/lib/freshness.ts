/**
 * Prefer a normalized provider publication date. Legacy snippet parsing is
 * preserved for compatibility, but the Korean pipeline passes an empty snippet.
 */
...
export function formatPublicationAge({ publishedDate, ageHours }: Publication): string | null {
  ...
  if (ageHours < 24 * 14) return `${Math.round(ageHours / 24)}일 전`;
  return `${Math.round(ageHours / (24 * 7))}주 전`;
}
