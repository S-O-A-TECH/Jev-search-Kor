  ...
  return (
    <article className={cn('group', minor ? 'pl-4 border-l' : '')}>
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        {!age && <span className="shrink-0">· 게시일 불명</span>}
        {age && <span className="shrink-0">· <time dateTime={item.publishedDate}>{age}</time></span>}
        {item.engines.length > 1 && (
          <span title={item.engines.join(' + ')}>· 검색 공급자 {item.engines.length}곳</span>
        )}
      </div>
      <a
        className={cn(
      ...
      <div className="mt-1 flex items-center gap-3 text-xs text-muted-foreground">
        <span
          ...
          title="질문과의 관련성 확률 (정답 정확도가 아닙니다)"
        >
          <span
            className={cn(
          ...
          />
          {item.ranked ? `관련성 ${Math.round(item.relevance * 100)}%` : 'Jev 평가 미완료'}
        </span>
      </div>
    </article>
  );
  ...
  if (clusters.length === 0) {
    ...
    return (
      <p className="mt-8 text-muted-foreground">
        검색 결과가 없습니다. 검색 기간이나 공급자를 확인해 주세요.
      </p>
    );
  }
      ...
      {offTopic.length > 0 && !streaming && (
        <div className="mt-8">
          <button
            ...
            type="button"
          >
            관련성이 낮은 결과 {offTopic.length}건 {showOffTopic ? '접기' : '보기'}
          </button>
